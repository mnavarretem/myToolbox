function st_Stats = fn_multcompcluster(vt_input)
...
for kk = 1:size(mx_combinations,1)
    ...
    if numel(vt_dGr1) < 5 || numel(vt_dGr2) < 5
        mx_pValues(nm_iGr1,nm_iGr2)	= 1;
        mx_tValues(nm_iGr1,nm_iGr2)	= 0;
        mx_dof(nm_iGr1,nm_iGr2)		= 0;
        
        mx_pValues(nm_iGr2,nm_iGr1)	= 1;
    ...
    mx_pValues(nm_iGr1,nm_iGr2)	= tcdf(abs(nm_tTm),nm_dofTm,'upper');
    ...
    [nm_tTm,nm_dofTm]	= f_WelchTvalue(vt_dGr2,vt_dGr1);
    
    mx_pValues(nm_iGr2,nm_iGr1)	= tcdf(abs(nm_tTm),nm_dofTm,'upper');
